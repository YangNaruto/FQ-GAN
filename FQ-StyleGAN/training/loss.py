...
def D_logistic_simplegp(G, D, opt, training_set, minibatch_size, reals, labels, r1_gamma=10.0, r2_gamma=0.0): # pylint: disable=unused-argument
    ...
    if isinstance(real_scores_out, tuple):
        ...
        # ppl_real = autosummary('Perplexity/real', ppl_real)
        # ppl_fake = autosummary('Perplexity/fake', ppl_fake)
        real_quant_loss = autosummary('Loss/scores/quant_real', real_quant_loss)
        fake_quant_loss = autosummary('Loss/scores/quant_fake', fake_quant_loss)

        real_scores_out = autosummary('Loss/scores/real', real_scores_out)
        fake_scores_out = autosummary('Loss/scores/fake', fake_scores_out)
        loss = tf.nn.softplus(fake_scores_out)  # -log(1 - logistic(fake_scores_out))
        loss += tf.nn.softplus(-real_scores_out) + real_quant_loss + fake_quant_loss# -log(logistic(real_scores_out)) # temporary pylint workaround # pylint: disable=invalid-unary-operand-type
    else:
        real_scores_out = autosummary('Loss/scores/real', real_scores_out)
        fake_scores_out = autosummary('Loss/scores/fake', fake_scores_out)
        loss = tf.nn.softplus(fake_scores_out)  # -log(1 - logistic(fake_scores_out))
    ...
    return loss, (ppl_fake+ppl_real) / 2
