    ...
    for gpu in range(submit_config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.name_scope('D_loss'), tf.control_dependencies(lod_assign_ops):
                D_loss, perplexity = dnnlib.util.call_func_by_name(G=G_gpu, D=D_gpu, opt=D_opt, training_set=training_set, minibatch_size=minibatch_split, reals=reals, labels=labels, **D_loss_args)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        for _mb_repeat in range(minibatch_repeats):
            for _D_repeat in range(D_repeats):
                _, _, ppl = tflib.run([D_train_op, Gs_update_op, perplexity], {lod_in: sched.lod, lrate_in: sched.D_lrate, minibatch_in: sched.minibatch})
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            # Report progress.
            print('tick %-5d kimg %-8.1f lod %-5.2f minibatch %-4d time %-12s sec/tick %-7.1f sec/kimg %-7.2f maintenance %-6.1f gpumem %-4.1f, ppl %-4.4f' % (
