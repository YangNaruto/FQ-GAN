...
def get_images(filename):
    x = cv2.imread(filename,flags=cv2.IMREAD_COLOR)
    x = cv2.cvtColor(x, cv2.COLOR_BGR2RGB)
    x = cv2.resize(x, dsize=(299, 299))
    return x


def kernel_inception_distance(fake_path, real_tgt_path, perfix) :
    filenames = glob(os.path.join(real_tgt_path, '*.*'))
    ...
    kid_score = '{0}: KID_mean: {1:3f}, KID_stddev: {2:3f}'.format(perfix, KID_mean*100,
                                                                   KID_stddev*100)
    print(kid_score)
    return kid_score


def mean_kernel_inception_distance(fake_path, real_src_path, real_tgt_path, perfix):
    source_alpha = 0.98
    target_alpha = 1 - source_alpha

    filenames = glob(os.path.join(real_src_path, '*.*'))
    ...
    n_src = real_source_images.shape[0]

    filenames = glob(os.path.join(real_tgt_path, '*.*'))
    real_target_images = [get_images(filename) for filename in filenames]
    real_target_images = np.random.permutation(np.transpose(real_target_images, axes=[0, 3, 1,
                                                                                      2]))[:n_src]
    ...
    mean_kid_score = '{0}: KID_mean: {1:3f}, KID_stddev: {2:3f}, mean_KID_mean: {3:3f}, ' \
                     'mean_KID_stddev: {4:3f}'.format(perfix, KID_mean*100, KID_stddev*100,
                                                      mean_KID_mean*100, mean_KID_stddev*100)
    print(mean_kid_score)
    return mean_kid_score


if __name__ == "__main__":
    kid_score = kernel_inception_distance(real_tgt_path='./real_target', fake_path='./fake',
                                          perfix=None)
    print(kid_score)